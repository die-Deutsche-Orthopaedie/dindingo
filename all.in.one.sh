...
########################                          Parameter Analysis                           ########################
parameters=`getopt -o dhDSTFH -a -l deep-analysis,download,sidefield,thread,files,help -- "$@"`
...
do  
    case "$1" in  
		...
		-h | -H | --help)  
			...
			echo "	-d or --deep-analysis: deep analysis mode, will download file info from colayun.com aka coladisk.com but NOT download any file from coladisk"
			echo "	-D or --download: download mode, will download file info AND file from colayun.com aka coladisk.com"
			echo
			...
			echo
			echo "help: "
			echo "	-H or -h or --help: ask for help"
			echo
			...
			echo "ps. "
			echo "	长发都tm是怪物"
			echo "	long hair girls are all fuckin' monsters"
			echo "	短发才tm是萌妹"
...
if [ -z "$parameter" ]
then
	echo "You MUST at least provide a parameter"
	exit 1
fi
...
{	
	...
	then
		...
		echo $downloaded | grep -Eo "<title>.*</title>" | sed 's/<title>//g ' | sed 's/<\/title>//g ' | sed 's/ - 丁丁购 - Powered by phpwind//g '
		echo
		########################                          title ends                           ########################
